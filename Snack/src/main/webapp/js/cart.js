    ...
    .then((res) => {
      ...
      $(res).each((idx, cart) => {
        let tr = `      <tr>
                                    ...
                                    <td><button id="modBtn" size="10px">수정</td>
                                    <td class="shoping__cart__item__close" id="delCart"">
                                        <span class="icon_close" data-cartCode="${
                                          cart.cartCode
                                        }"></span>
                                        </td>
                                        </tr>`;
      ...
      $("#modBtn").on("click", function () {
        console.log(event.target.closest("span").data("cartcode"));
        fetch("modCart.do?cartCode=" + cartcode)
          .then((result) => result.json())
          .then((result) => {
            if (result.retCode == "OK") {
              alert("수정완료");
